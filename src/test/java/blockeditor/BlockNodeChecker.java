...
public class BlockNodeChecker {
	...
	public void test() throws SAXException, IOException {
		...
		for (String key : map.keySet()) {
			...
			if (MyDOMUtil.getAttribute(node, BlockElementModel.KIND_ATTR).equals("function")) {
				if (MyDOMUtil.getChildText(node, "Type") == null) {
					if (!key.equals("agent") && !key.equals("callGetterMethod2")) {
						throw new RuntimeException(key);
					}
				}

				if (!key.equals("callGetterMethod2") &&!key.equals("special-expression") &&!key.equals("type-object")&&!key.equals("agent") && MyDOMUtil.getChildText(node, "Name") == null && !isBinOp(node) && !key.startsWith("new-") && resolver.getForceConvertionMap().getUniMethodCallModel(key) == null) {
					throw new RuntimeException(key);
				}

			} else if (MyDOMUtil.getAttribute(node, BlockElementModel.KIND_ATTR).equals("command")) {
				if (MyDOMUtil.getChildText(node, "Name") == null) {
